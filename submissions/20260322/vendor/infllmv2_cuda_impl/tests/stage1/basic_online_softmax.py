...
def apply_causal_mask(S, qi, kj, max_k_len, causal=False):
    for _i, i in enumerate(range(qi * block_q, (qi + 1) * block_q)):
        ...
        for _j, j in enumerate(range(kj * block_k, (kj + 1) * block_k)):
            if j < left_border or j >= right_border or j >= max_k_len:
                S[:, _i, _j] = float('-inf')
    return S

def torch_online_softmax(q, k, block_q, block_k, seq_dim=0, causal=False):
    device = q.device
    dtype = q.dtype
    ...
    if k_len % 16 != 0:
        k = torch.cat([k, torch.zeros(16 - k_len % 16, k_head_n, d, device=device, dtype=dtype)], dim=0)
        max_k_len = k_len
    ...
    out = out.reshape(head_n_k, group_size, q_len, k_len).sum(dim=1) 
    out = torch.where(torch.isnan(out), 0, out)[:, :q_len, :max_k_len]
