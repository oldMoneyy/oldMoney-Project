...
class SimpleGLAAttnBackend(MambaAttnBackendBase):
    ...
    def forward(
        ...
        # Load initial state.
        # For decode: always needed (short-circuit avoids .any() GPU sync).
        # For prefill: only needed if there are prefix lengths > 0.
        initial_state = None
        need_state = is_decode or (
            forward_batch.extend_prefix_lens is not None
            and forward_batch.extend_prefix_lens.any()
        )
        ...
        if need_state:
            if cache_idx is not None:
                ...
                # Advanced indexing already returns a contiguous copy — no .contiguous() needed
                initial_state = layer_cache.temporal[mamba_indices, :]
            else:
                raise RuntimeError(
        ...
        # Save final state — reuse cache_idx and mamba_indices (no redundant lookups)
