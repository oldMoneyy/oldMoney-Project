...
class HybridLinearKVPool(KVCache):
    ...
    def __init__(
        ...
        if not use_mla:
            ...
            if fp4_compress:
                TokenToKVPoolClass = MHATokenToKVPoolFP4Compressed
                logger.info("HybridLinearKVPool: using FP4 compressed storage with FP8 presentation")
            elif is_float4_e2m1fn_x2(dtype):
    ...
    def prepare_for_layer(self, layer_id: int, active_indices: torch.Tensor = None):
        """Forward prepare_for_layer to the underlying KV pool if it supports it."""
        mapped_id = self._transfer_full_attention_id(layer_id)
        if hasattr(self.full_kv_pool, 'prepare_for_layer'):
            self.full_kv_pool.prepare_for_layer(mapped_id, active_indices)
...
class MHATokenToKVPoolMixedFP4FP8(MHATokenToKVPool):
    """Mixed precision KV cache: FP8 for sensitive layers, FP4 for the rest."""

    def __init__(self, *args, fp8_layer_ids=None, **kwargs):
        self.fp8_layer_ids = set(fp8_layer_ids or [])
        ...
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                for i in range(self.layer_num):
                    abs_layer = i + self.start_layer
                    is_fp8 = abs_layer in self.fp8_layer_ids
                    self._layer_is_fp8.append(is_fp8)

                    if is_fp8:
                        self.k_buffer.append(torch.zeros((m, n, k), dtype=self.store_dtype, device=self.device))
                        self.v_buffer.append(torch.zeros((m, n, k), dtype=self.store_dtype, device=self.device))
                        self.k_scale_buffer.append(None)
                        self.v_scale_buffer.append(None)
                    else:
                        self.k_buffer.append(torch.zeros((m, n, k // 2), dtype=self.store_dtype, device=self.device))
                        self.v_buffer.append(torch.zeros((m, n, k // 2), dtype=self.store_dtype, device=self.device))
                        self.k_scale_buffer.append(torch.zeros((m, (n * k) // scale_block_size), dtype=self.store_dtype, device=self.device))
                        self.v_scale_buffer.append(torch.zeros((m, (n * k) // scale_block_size), dtype=self.store_dtype, device=self.device))

        # Build data_ptrs/strides for parent (used by move_kv_cache)
    ...
    def _clear_buffers(self):
        del self.k_buffer, self.v_buffer, self.k_scale_buffer, self.v_scale_buffer

    def _get_key_buffer(self, layer_id: int):
        idx = layer_id - self.start_layer
        if self._layer_is_fp8[idx]:
            return self.k_buffer[idx].view(self.dtype)
        else:
            from sglang.srt.layers.quantization.kvfp4_tensor import KVFP4QuantizeUtil
            return KVFP4QuantizeUtil.batched_dequantize(
                self.k_buffer[idx].view(torch.uint8),
    ...
    def _get_value_buffer(self, layer_id: int):
        idx = layer_id - self.start_layer
        if self._layer_is_fp8[idx]:
            return self.v_buffer[idx].view(self.dtype)
        else:
            from sglang.srt.layers.quantization.kvfp4_tensor import KVFP4QuantizeUtil
            return KVFP4QuantizeUtil.batched_dequantize(
                self.v_buffer[idx].view(torch.uint8),
    ...
    def set_kv_buffer(
        ...
        if self._layer_is_fp8[idx]:
            # FP8 path
            cache_k = cache_k.to(self.dtype).view(self.store_dtype)
            cache_v = cache_v.to(self.dtype).view(self.store_dtype)
            self.k_buffer[idx][loc] = cache_k
            self.v_buffer[idx][loc] = cache_v
        else:
            # FP4 path
            from sglang.srt.layers.quantization.kvfp4_tensor import KVFP4QuantizeUtil
            cache_k_q, cache_k_sf = KVFP4QuantizeUtil.batched_quantize(cache_k)
            cache_v_q, cache_v_sf = KVFP4QuantizeUtil.batched_quantize(cache_v)
            self.k_buffer[idx][loc] = cache_k_q.view(self.store_dtype)
            self.v_buffer[idx][loc] = cache_v_q.view(self.store_dtype)
            self.k_scale_buffer[idx][loc] = cache_k_sf.view(self.store_dtype)
            self.v_scale_buffer[idx][loc] = cache_v_sf.view(self.store_dtype)


class MHATokenToKVPoolFP4Compressed(MHATokenToKVPool):
    """Stores KV cache in FP4 internally but presents FP8 to attention backends.

    Allocates:
      - FP4 packed buffers (half-size) + scale buffers for ALL layers
      - ONE shared FP8 scratch buffer for K and ONE for V (reused across layers)

    On get_key/value_buffer: dequantize FP4 -> FP8 into scratch, return scratch.
    On set_kv_buffer: quantize input -> FP4 and store.
    """
    ...
    def _create_buffers(self):
        with self.memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            with (
                ...
                # FP4 packed storage (half-size per layer)
                self.k_buffer_fp4 = [
                    torch.zeros((m, n, k // 2), dtype=torch.uint8, device=self.device)
                    for _ in range(self.layer_num)
                ]
                self.v_buffer_fp4 = [
                    torch.zeros((m, n, k // 2), dtype=torch.uint8, device=self.device)
                ...
                self.k_scale_buffer = [
                    torch.zeros((m, (n * k) // scale_block_size), dtype=torch.uint8, device=self.device)
                    for _ in range(self.layer_num)
                ]
                self.v_scale_buffer = [
                    torch.zeros((m, (n * k) // scale_block_size), dtype=torch.uint8, device=self.device)
    ...
    def _clear_buffers(self):
        del self.k_buffer_fp4, self.v_buffer_fp4
        del self.k_scale_buffer, self.v_scale_buffer
        del self.k_buffer, self.v_buffer

    def get_kv_size_bytes(self):
        k_size = sum(
            get_tensor_size_bytes(b) + get_tensor_size_bytes(s)
            for b, s in zip(self.k_buffer_fp4, self.k_scale_buffer)
        ) + get_tensor_size_bytes(self._k_scratch)
        v_size = sum(
            get_tensor_size_bytes(b) + get_tensor_size_bytes(s)
            for b, s in zip(self.v_buffer_fp4, self.v_scale_buffer)
        ) + get_tensor_size_bytes(self._v_scratch)
        return k_size, v_size

    def _get_key_buffer(self, layer_id: int):
        from sglang.srt.layers.quantization.fp4_to_fp8_triton import fp4_to_fp8_triton as _fp4_to_fp8
        idx = layer_id - self.start_layer
        _fp4_to_fp8(
            self.k_buffer_fp4[idx], self.k_scale_buffer[idx], self._k_scratch
        )
        return self._k_scratch.view(self.dtype)

    def _get_value_buffer(self, layer_id: int):
        from sglang.srt.layers.quantization.fp4_to_fp8_triton import fp4_to_fp8_triton as _fp4_to_fp8
        idx = layer_id - self.start_layer
        _fp4_to_fp8(
            self.v_buffer_fp4[idx], self.v_scale_buffer[idx], self._v_scratch
        )
        return self._v_scratch.view(self.dtype)
    ...
    def set_kv_buffer(
        ...
        # Quantize to FP4
        k_q, k_sf = KVFP4QuantizeUtil.batched_quantize(cache_k)
        v_q, v_sf = KVFP4QuantizeUtil.batched_quantize(cache_v)

        self.k_buffer_fp4[idx][loc] = k_q
        self.v_buffer_fp4[idx][loc] = v_q
        self.k_scale_buffer[idx][loc] = k_sf
        self.v_scale_buffer[idx][loc] = v_sf

    def move_kv_cache(self, tgt_loc, src_loc):
        """Move KV cache in FP4 storage directly (no dequant needed)."""
        if tgt_loc.numel() == 0:
            return
        tgt = tgt_loc.view(-1).long()
        src = src_loc.view(-1).long()
        for i in range(self.layer_num):
            self.k_buffer_fp4[i][tgt] = self.k_buffer_fp4[i][src]
            self.v_buffer_fp4[i][tgt] = self.v_buffer_fp4[i][src]
            self.k_scale_buffer[i][tgt] = self.k_scale_buffer[i][src]
            self.v_scale_buffer[i][tgt] = self.v_scale_buffer[i][src]
