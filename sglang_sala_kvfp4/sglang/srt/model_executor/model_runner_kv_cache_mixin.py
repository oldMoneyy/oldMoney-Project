...
from sglang.srt.mem_cache.memory_pool import (
    ...
    MHATokenToKVPool,
    MHATokenToKVPoolFP4,
    MHATokenToKVPoolMixedFP4FP8,
    MHATokenToKVPoolFP4Compressed,
    MLATokenToKVPool,
    MLATokenToKVPoolFP4,
    NSATokenToKVPool,
    ReqToTokenPool,
...
class ModelRunnerKVCacheMixin:
        ...
        if self.server_args.attention_backend == "ascend":
            ...
            if self.is_hybrid_swa:
                ...
                self.token_to_kv_pool = HybridLinearKVPool(
                    ...
                    fp4_compress=getattr(self.server_args, 'kv_cache_fp4_compress', False),
                    **extra_args,
                )
            else:
                if is_float4_e2m1fn_x2(self.kv_cache_dtype):
                    fp8_layer_ids = self._parse_kv_fp8_layers()
                    if fp8_layer_ids:
                        self.token_to_kv_pool = MHATokenToKVPoolMixedFP4FP8(
                            self.max_total_num_tokens,
                            page_size=self.page_size,
                            dtype=self.kv_cache_dtype,
                            head_num=self.model_config.get_num_kv_heads(
                            ...
                            fp8_layer_ids=fp8_layer_ids,
                        )
                        logger.info(f"Mixed FP4/FP8 KV cache: FP8 layers={sorted(fp8_layer_ids)}")
                    else:
    ...
    def _parse_kv_fp8_layers(self: "ModelRunner"):
        raw = getattr(self.server_args, "kv_cache_fp8_layers", None)
        if not raw:
            return []
        if raw.endswith(".json"):
            ...
            with open(raw) as f:
                data = json.load(f)
            return data["fp8_layers"]
        return [int(x.strip()) for x in raw.split(",")]

    # keep original log at end of init_memory_pool
    def _log_memory_pool_end(self: "ModelRunner"):
        logger.info(
            f"Memory pool end. "
