...
from sglang.srt.utils.sala_profiler import get_profiler, is_profiling_enabled
...
class Scheduler(
    ...
    def run_batch(
        ...
        # SALA profiling: begin step
        profiler = get_profiler()
        if profiler is not None:
            forward_mode_str = str(batch.forward_mode) if batch else "unknown"
            batch_size = len(batch.reqs) if batch and hasattr(batch, 'reqs') else 0
            num_tokens = int(sum(batch.seq_lens)) if batch and hasattr(batch, 'seq_lens') else 0
            profiler.begin_step(
        ...
        # SALA profiling: end step
        if profiler is not None:
            profiler.record_timing("total_step_ms", (time.perf_counter() - step_start) * 1000)
            profiler.end_step()
