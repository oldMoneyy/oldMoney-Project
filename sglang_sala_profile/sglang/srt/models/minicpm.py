...
from sglang.srt.utils.sala_profiler import CudaTimer, get_profiler, is_profiling_enabled
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if profiler is not None:
            ...
            # MLP (includes post_attention_layernorm)
            residual = hidden_states
            with mlp_timer:
            ...
            # Sync and record
            ln_ms = ln_timer.sync_and_get_ms()
            attn_ms = attn_timer.sync_and_get_ms()
            mlp_ms = mlp_timer.sync_and_get_ms()
            profiler.record_layer(self.layer_id, self.mixer_type, ln_ms, attn_ms, mlp_ms)
        else:
            # Original path - zero overhead
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)
            hidden_states = self.self_attn(
                positions=positions,
...
class MiniCPMModel(nn.Module):
    ...
    def forward(
        ...
        if profiler is not None:
            embed_timer = CudaTimer()
            norm_timer = CudaTimer()
            ...
            profiler.record_timing("embed_ms", embed_timer.sync_and_get_ms())
            profiler.record_timing("final_norm_ms", norm_timer.sync_and_get_ms())
        else:
            if input_embeds is None:
                hidden_states = self.embed_tokens(input_ids) * self.config.scale_emb
            else:
