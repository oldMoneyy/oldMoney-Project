...
def fused_recurrent_fwd_kernel(
    ...
    if USE_INITIAL_STATE:
        if USE_H0_INDICES:
            h0_idx = tl.load(h0_indices + i_n).to(tl.int64)
            p_h0 = h0 + (h0_idx * H + i_h) * K * V + o_k[:, None] * V + o_v[None, :]
        else:
    ...
    if STORE_FINAL_STATE:
        if USE_H0_INDICES:
            ht_idx = tl.load(h0_indices + i_n).to(tl.int64)
            p_ht = h0 + (ht_idx * H + i_h) * K * V + o_k[:, None] * V + o_v[None, :]
        else:
