...
class SimpleGLAAttnBackend(MambaAttnBackendBase):
    ...
    def __init__(self, model_runner: ModelRunner):
        ...
        # llmv fix: fla's g_gamma is log-space decay, not -slopes.
        # Original SGLang upstream used -slopes which diverges from
        # the training-side g=log(slopes). See parity test in
        # nanochat/linear_attention.py. Divergence up to 35x.
        self.g_gamma = torch.log(
            _build_slope_tensor(num_heads).to(dtype=torch.float32, device=self.device)
        )  # (h,) — log-space ALiBi decay per head
