...
class MiniCPMLightningMixer(nn.Module):
    ...
    def forward(
        ...
        # Backend validated once at init; skip per-forward isinstance checks
        ...
        # Backend already reshapes to (-1, num_heads * head_dim), skip redundant reshape
...
class MiniCPMModel(nn.Module):
    ...
    def forward(
        ...
        if input_embeds is None:
            ...
            # scale_emb is pre-baked into embed weights by MiniCPMForCausalLM._prescale_weights()
        else:
...
class MiniCPMForCausalLM(nn.Module):
    ...
    def forward(
        ...
        # scale_width is pre-baked into lm_head weights by _prescale_weights()
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        # Pre-scale weights to eliminate per-forward element-wise ops
        self._prescale_weights()

    def _prescale_weights(self):
        """Bake scaling factors into weights to eliminate per-forward ops.

        - scale_emb into embed_tokens weights (eliminates hidden_states.mul_())
        - 1/scale_width into lm_head weights (eliminates hidden_states.div_())
        """
        with torch.no_grad():
            self.model.embed_tokens.weight.data.mul_(self.config.scale_emb)
            if not self.config.tie_word_embeddings:
                self.lm_head.weight.data.div_(self.scale_width)
            else:
                # tie_word_embeddings: embed already scaled, logits will use it directly
                # We still need to handle the div by scale_width.
                # Since embed weights are shared, we can't bake both into the same weight.
                # For tied embeddings, keep the div_ in forward.
                pass
