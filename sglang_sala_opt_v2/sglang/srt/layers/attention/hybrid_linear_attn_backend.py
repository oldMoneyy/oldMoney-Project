...
class SimpleGLAAttnBackend(MambaAttnBackendBase):
    ...
    def forward(
        ...
        # Fast path: decode always needs state; prefill only if has prefix
        need_state = is_decode
        if not need_state:
            prefix_lens = forward_batch.extend_prefix_lens
            need_state = prefix_lens is not None and (prefix_lens > 0).any()
        ...
        if is_decode or (not is_decode and torch.max(forward_batch.extend_seq_lens) < 64):
        ...
        if final_state is not None:
            # Reuse mamba_indices and cache_idx from above (no redundant recomputation)
            if cache_idx is not None:
                layer_cache = self.req_to_token_pool.mamba_pool.mamba2_layer_cache(cache_idx)
