...
class SimpleGLAAttnBackend(MambaAttnBackendBase):
    ...
    def forward(
        ...
        lid = f"L{layer_id:02d}"
        mode = "decode" if is_decode else "prefill"

        with profile_region(f"{lid}/lightning/fla_state_load/{mode}"):
            ...
            if is_decode or has_initial_state.any():
        ...
        mode_str = "fused_recurrent" if seq_len < 64 else "chunk"
        if is_decode or mode_str == "fused_recurrent":
            kernel_name = f"{lid}/lightning/fused_recurrent_gla/{mode}"
            with profile_region(kernel_name):
            ...
            kernel_name = f"{lid}/lightning/chunk_simple_gla/{mode}"
            with profile_region(kernel_name):
        ...
        with profile_region(f"{lid}/lightning/fla_state_save/{mode}"):
