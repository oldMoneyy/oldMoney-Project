...
# Profiling support
import sys as _sys
_sys.path.insert(0, str(__import__('pathlib').Path(__file__).resolve().parent.parent.parent.parent))
...
class MiniCPMSparseBackend(AttentionBackend):
    ...
    def get_topk_for_sparse(
        ...
        if is_prefill:
            ...
            with profiler.timer(f"layer.{layer.layer_id}.sparse.key_compress"):
            ...
            with profiler.timer(f"layer.{layer.layer_id}.sparse.topk_metadata"):
            ...
            with profiler.timer(f"layer.{layer.layer_id}.sparse.topk_impl"):
                if self.enable_cuda_graph:
                    ret = self.sparse_get_topk_impl(
                        query_states,
                        cu_seqlens_q,
    ...
    def forward_decode(
        ...
        bs = forward_batch.batch_size
        with profiler.timer(f"layer.{layer.layer_id}.sparse.kv_cache_write"):
            if k is not None:
                assert v is not None
                if save_kv_cache:
                    cache_loc = forward_batch.out_cache_loc
        ...
        with profiler.timer(f"layer.{layer.layer_id}.sparse.topk_total"):
            topk_idx = self.get_topk_for_sparse(
        ...
        with profiler.timer(f"layer.{layer.layer_id}.sparse.block_table"):
        ...
        # Use the attention kernel abstraction
        with profiler.timer(f"layer.{layer.layer_id}.sparse.flashinfer_attn"):
            result = self.attention_kernel.forward(attn_params, layer)
