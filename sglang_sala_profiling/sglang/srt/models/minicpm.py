...
class MiniCPMAttention(nn.Module):
    ...
    def forward(
        ...
        lid = self.layer_id
        with profiler.timer(f"layer.{lid}.minicpm4.qkv_proj"):
            qkv, _ = self.qkv_proj(hidden_states)
            q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)

        with profiler.timer(f"layer.{lid}.minicpm4.rope"):
            if self.attn_use_rope:
                orig_dtype = q.dtype
                q, k = q.float(), k.float()
        ...
        with profiler.timer(f"layer.{lid}.minicpm4.attn"):
            attn_output = self.attn(q, k, v, forward_batch)

        with profiler.timer(f"layer.{lid}.minicpm4.output_gate_and_proj"):
            if self.use_output_gate:
...
class MiniCPMLightningMixer(nn.Module):
    ...
    def forward(
        ...
        lid = self.layer_id
        with profiler.timer(f"layer.{lid}.lightning.qkv_proj"):
            qkv, _ = self.qkv_proj(hidden_states)
            q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)

        with profiler.timer(f"layer.{lid}.lightning.qk_norm"):
            if self.qk_norm:
                q = self.q_norm(q.reshape(-1, self.head_dim))
                k = self.k_norm(k.reshape(-1, self.head_dim))

        with profiler.timer(f"layer.{lid}.lightning.rope"):
            if self.use_rope:
        ...
        with profiler.timer(f"layer.{lid}.lightning.reshape"):
            q = q.reshape(-1, self.num_heads, self.head_dim)
            k = k.reshape(-1, self.num_kv_heads, self.head_dim)
        ...
        with profiler.timer(f"layer.{lid}.lightning.gla_kernel"):
        ...
        with profiler.timer(f"layer.{lid}.lightning.output_norm_gate_proj"):
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        with profiler.timer(f"layer.{lid}.{mtype}.input_layernorm"):
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)

        # self_attn is already instrumented internally
        ...
        with profiler.timer(f"layer.{lid}.{mtype}.residual_add_1"):
            hidden_states = residual + hidden_states * (
                self.config.scale_depth / math.sqrt(self.config.num_hidden_layers)
            )

        with profiler.timer(f"layer.{lid}.{mtype}.post_attn_layernorm"):
            residual = hidden_states
            hidden_states = self.post_attention_layernorm(hidden_states)

        with profiler.timer(f"layer.{lid}.{mtype}.mlp"):
            hidden_states = self.mlp(hidden_states)

        with profiler.timer(f"layer.{lid}.{mtype}.residual_add_2"):
            hidden_states = residual + hidden_states * (
                self.config.scale_depth / math.sqrt(self.config.num_hidden_layers)
            )
...
class MiniCPMForCausalLM(nn.Module):
    ...
    def forward(
        ...
        with profiler.timer("logits_processor"):
