...
class MiniCPMAttention(nn.Module):
    ...
    def forward(
        ...
        from sglang.srt.sala_profiler import profile_region

        lid = f"L{self.layer_id:02d}"
        mode = "decode" if forward_batch.forward_mode.is_decode() else "prefill"

        with profile_region(f"{lid}/minicpm4/qkv_proj/{mode}"):
            qkv, _ = self.qkv_proj(hidden_states)
            q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)

        if self.attn_use_rope:
            with profile_region(f"{lid}/minicpm4/rope/{mode}"):
                orig_dtype = q.dtype
                q, k = q.float(), k.float()
                q, k = self.rotary_emb(positions, q, k)
                q, k = q.to(orig_dtype), k.to(orig_dtype)

        with profile_region(f"{lid}/minicpm4/attn_kernel/{mode}"):
            attn_output = self.attn(q, k, v, forward_batch)

        if self.use_output_gate:
            with profile_region(f"{lid}/minicpm4/o_gate/{mode}"):
                o_gate_output, _ = self.o_gate(hidden_states)
                attn_output = attn_output * F.sigmoid(o_gate_output)

        with profile_region(f"{lid}/minicpm4/o_proj/{mode}"):
...
class MiniCPMLightningMixer(nn.Module):
    ...
    def forward(
        ...
        with profile_region(f"{lid}/lightning/qkv_proj/{mode}"):
            qkv, _ = self.qkv_proj(hidden_states)
            q, k, v = qkv.split([self.q_size, self.kv_size, self.kv_size], dim=-1)

        if self.qk_norm:
            with profile_region(f"{lid}/lightning/qk_norm/{mode}"):
                q = self.q_norm(q.reshape(-1, self.head_dim))
                k = self.k_norm(k.reshape(-1, self.head_dim))

        if self.use_rope:
            with profile_region(f"{lid}/lightning/rope/{mode}"):
        ...
        with profile_region(f"{lid}/lightning/fla_kernel/{mode}"):
        ...
        if self.use_output_norm:
            with profile_region(f"{lid}/lightning/o_norm/{mode}"):
                o = self.o_norm(o)

        if self.use_output_gate:
            with profile_region(f"{lid}/lightning/sigmoid_gate/{mode}"):
                z, _ = self.z_proj(hidden_states)
                o = o * F.sigmoid(z)

        with profile_region(f"{lid}/lightning/o_proj/{mode}"):
            y, _ = self.o_proj(o)
        return y
...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        lid = f"L{self.layer_id:02d}"
        mtype = self.mixer_type
        mode = "decode" if forward_batch.forward_mode.is_decode() else "prefill"

        with profile_region(f"{lid}/{mtype}/input_norm/{mode}"):
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)

        with profile_region(f"{lid}/{mtype}/attn/{mode}"):
        ...
        with profile_region(f"{lid}/{mtype}/post_attn_norm/{mode}"):
        ...
        with profile_region(f"{lid}/{mtype}/mlp/{mode}"):
...
class MiniCPMModel(nn.Module):
    ...
    def forward(
        ...
        with profile_region(f"model/embed/{mode}"):
        ...
        with profile_region(f"model/final_norm/{mode}"):
...
class MiniCPMForCausalLM(nn.Module):
    ...
    def forward(
        ...
        from sglang.srt.sala_profiler import profile_region, step_completed
        ...
        with profile_region(f"model/logits/{mode}"):
        ...
        step_completed(mode=mode, num_tokens=input_ids.shape[0])
        return result
