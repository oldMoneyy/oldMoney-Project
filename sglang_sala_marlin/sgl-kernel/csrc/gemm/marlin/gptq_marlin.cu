...
// --- Multi-tier tile configs for MiniCPM-SALA ---
// Decode (M=1): memory-bound, prefer large K tiles for bandwidth utilization
thread_config_t decode_small_n_thread_configs[] = {
    // For N <= 256 (minicpm4 k/v_proj: K=4096, N=256)
    // Small N -> fewer N-tiles, maximize K to fill pipeline
    {128, 64, 128},
    {64, 128, 128},
    {128, 128, 256}};

thread_config_t decode_medium_n_thread_configs[] = {
    // For N <= 4096 (most projections: K=4096, N=4096; down_proj: K=16384, N=4096)
    // Balanced K/N, lean toward K for bandwidth
    {128, 128, 256},
    {128, 64, 128},
    {64, 128, 128}};

thread_config_t decode_large_n_thread_configs[] = {
    // For N > 4096 (gate/up_proj: K=4096, N=16384)
    // Large N -> more N-parallelism benefits
    {64, 128, 128},
    {128, 128, 256},
    {128, 64, 128}};

// Small prefill (M=2..16): transitional zone
thread_config_t small_prefill_thread_configs[] = {
    {128, 128, 256},
    {64, 128, 128},
    {128, 64, 128}};

// Large prefill (M>16): compute-bound, prefer large N tiles for parallelism
thread_config_t large_prefill_small_n_thread_configs[] = {
    // N <= 4096
    {64, 128, 128},
    {64, 256, 256},
    {128, 64, 128}};

thread_config_t large_prefill_large_n_thread_configs[] = {
    // N > 4096 (gate/up_proj)
    {64, 256, 256},
    {64, 128, 128},
    {128, 64, 128}};
...
exec_config_t determine_exec_config(
  ...
  // Multi-tier tile config selection based on M and N dimensions
  thread_config_t* thread_configs;
  int thread_configs_size;

  if (thread_m_blocks <= 1) {
    // Decode path (M <= 16, typically M=1..8)
    if (prob_n <= 256) {
      thread_configs = decode_small_n_thread_configs;
      thread_configs_size = sizeof(decode_small_n_thread_configs) / sizeof(thread_config_t);
    } else if (prob_n <= 4096) {
      thread_configs = decode_medium_n_thread_configs;
      thread_configs_size = sizeof(decode_medium_n_thread_configs) / sizeof(thread_config_t);
    } else {
      thread_configs = decode_large_n_thread_configs;
      thread_configs_size = sizeof(decode_large_n_thread_configs) / sizeof(thread_config_t);
    }
  } else if (thread_m_blocks <= 2) {
    // Small prefill (M=17..32)
    thread_configs = small_prefill_thread_configs;
    thread_configs_size = sizeof(small_prefill_thread_configs) / sizeof(thread_config_t);
  } else {
    // Large prefill (M>32)
    if (prob_n <= 4096) {
      thread_configs = large_prefill_small_n_thread_configs;
      thread_configs_size = sizeof(large_prefill_small_n_thread_configs) / sizeof(thread_config_t);
    } else {
      thread_configs = large_prefill_large_n_thread_configs;
      thread_configs_size = sizeof(large_prefill_large_n_thread_configs) / sizeof(thread_config_t);
    }
  }
