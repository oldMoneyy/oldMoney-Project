...
def quantize_model(args):
    ...
    # dotv
    inp_dir = f"/tmp/gptq_inps_oldmoney_{args.seed}"
    # inp_dir = os.path.join(os.path.dirname(args.output), "gptq_inps")
    ...
    with torch.no_grad():
        for i, sample in enumerate(calib_data):
            ...
            torch.save(hidden.cpu(), f"{inp_dir}/{i}.pt")  # all_inps.append(hidden.cpu())  # dotv
            all_masks.append(attention_mask.cpu())
            all_pos_ids.append(position_ids.cpu())

    embed = embed.cpu()
    nuke_all_caches()
    tmp = torch.load(f"{inp_dir}/0.pt")
    print(f"[INFO] Initial hidden states computed. Shape per sample: {tmp.shape}")
    del tmp
        ...
        if layer_bits >= 16:
            ...
            with torch.no_grad():
                for i in range(len(calib_data)):
                    inp = torch.load(f"{inp_dir}/{i}.pt").to(device)  # all_inps[i].to(device)  # dotv
                    ...
                    torch.save(out[0].cpu(), f"{inp_dir}/{i}.pt")
                    if (i + 1) % 16 == 0:
                        nuke_all_caches()
    ...
    save_quantized_model(
        quantized_state, original_state, args, config
    )
    shutil.rmtree(f"/tmp/gptq_inps_oldmoney_{args.seed}", ignore_errors=True)  # dotv
