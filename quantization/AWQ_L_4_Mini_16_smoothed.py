#!/usr/bin/env python3
"""
NVFP4 Quantization for MiniCPM-SALA with Channel Smoothing
============================================================

Architecture-aware quantization:
  - MiniCPM4 layers: Attention BF16 (fused QKV constraint), MLP FP4
  - Lightning-attn layers: Everything FP4, with channel smoothing

SGLang compatibility:
  - Fused QKV (q+k+v) must be all-quantized or all-excluded
  - Fused gate_up (gate+up) must be all-quantized or all-excluded
  - exclude_modules patterns match SGLang's is_layer_skipped + is_layer_excluded
  - MiniCPMDecoderLayer.is_excluded cascades: any self_attn sub-module
    exclusion triggers entire attention block to BF16
...
def apply_layer_smoothing(
    ...
    """
    Apply channel smoothing. Only for Lightning-attn layers.

    MiniCPM4 layers: attention is BF16, only MLP is quantized.
    We smooth post_attention_layernorm -> [gate, up] for MiniCPM4 MLPs.
    We do NOT smooth input_layernorm in MiniCPM4 layers because
    the attention weights stay BF16 and shouldn't be modified.

    Lightning-attn layers: full smoothing on all three groups.
    """
    smooth_factors = {}

    # --- Group 1: input_layernorm -> attention inputs (Lightning only) ---
    if mixer_type != "minicpm4":
        attn_targets = ["self_attn.q_proj", "self_attn.k_proj",
                        "self_attn.v_proj", "self_attn.z_proj"]
        attn_present = [t for t in attn_targets if t in linears]
        input_ln = get_submodule_safe(layer, "input_layernorm")

        if attn_present and input_ln is not None and hasattr(input_ln, "weight"):
            target_weights = [
                linears[t].weight.data.to(torch.float32) for t in attn_present
            ]
            h_diags = [
                observers[t].get_h_diag().to(target_weights[0].device)
                for t in attn_present
            ]
            ...
            for t in attn_present:
                ...
                smooth_factors[t] = s
            print(
                f"    Smooth [input_layernorm] -> {attn_present} "
                f"| s: min={s.min():.4f} max={s.max():.4f} std={s.std():.4f}"
            )

    # --- Group 2: post_attention_layernorm -> gate, up (ALL layers) ---
    ...
    # --- Group 3: o_norm -> o_proj (Lightning only) ---
    if mixer_type != "minicpm4":
        o_norm = get_submodule_safe(layer, "self_attn.o_norm")
        o_proj_name = "self_attn.o_proj"
        if (
            o_norm is not None
            and hasattr(o_norm, "weight")
            and o_proj_name in linears
        ):
            W = linears[o_proj_name].weight.data.to(torch.float32)
            h = observers[o_proj_name].get_h_diag().to(W.device)
            s = compute_smooth_factor([W], [h], alpha=alpha)
            o_norm.weight.data.div_(s.to(o_norm.weight.dtype))
            linears[o_proj_name].weight.data.mul_(
                s.unsqueeze(0).to(linears[o_proj_name].weight.dtype)
            )
            smooth_factors[o_proj_name] = s
            print(
...
def should_quantize_linear(
    ...
    """
    MiniCPM4: attention BF16 (fused QKV constraint), MLP FP4
    Lightning-attn: everything FP4

    SGLang fuses q_proj+k_proj+v_proj into qkv_proj.
    All three must be the same precision — no mixing allowed.
    MiniCPM4 has 2 KV heads (k/v are 256x4096, too small for FP4),
    so we must keep ALL of q,k,v in BF16 together.
    Similarly o_proj and o_gate stay BF16 since the entire attention
    block is set to BF16 by MiniCPMDecoderLayer.is_excluded cascade.
    """
    if mixer_type == "minicpm4":
        if "self_attn" in linear_name:
            return False
        return True  # MLP quantized
...
def build_exclude_modules(config) -> List[str]:
    ...
    We list all self_attn projections in MiniCPM4 layers explicitly.
    This satisfies the fused QKV all-or-nothing constraint.
    """
    exclude = []
...
def process_layer(
    ...
    # Phase 2: Channel smoothing
    print(f"  Phase 2: Channel smoothing (alpha={args.smooth_alpha})...")
    ...
    # Phase 6: Save non-linear params (layernorms, norms — includes smoothed values)
...
def save_checkpoint(
    ...
    # 1. FIX: Unpack the dictionaries correctly
    raw_state_dict = {**quantized_tensors, **original_tensors}
    
    # 2. FIX: Force contiguous memory to prevent Safetensors crash
...
def quantize_model(args):
    ...
    hs_store.cleanup()
    print(f"\n[DONE] NVFP4 quantization complete!")


def main():
    parser = argparse.ArgumentParser(
        description="NVFP4 Quantization for MiniCPM-SALA"
    )
    ...
    print("=" * 70)
    print("  NVFP4 Quantizer for MiniCPM-SALA")
    print("  MiniCPM4 attn=BF16 | MiniCPM4 MLP=FP4 | Lightning=FP4+smooth")
    print("=" * 70)
