#!/usr/bin/env python3
"""
Dense Flashinfer NVFP4 Quantization for MiniCPM-SALA
=====================================================
...
def apply_layer_smoothing(
    ...
    MiniCPM4 layers (attn=BF16, MLP=FP4):
      Group 1: post_attention_layernorm -> [gate_proj, up_proj]
      Group 2: up_proj -> [down_proj]
      (input_layernorm feeds into BF16 attention — don't touch)

    Lightning-attn layers (ALL=FP4):
    ...
    # --- Lightning-attn: smooth attention projections too ---
...
def should_quantize_linear(
    ...
    """
    Quantization decision per linear:

    Lightning-attn layers: quantize EVERYTHING (Q/K/V/Z/O + MLP)
      - With flashinfer backend, no GLA recurrence, FP4 is safe
      - Covers 24/32 layers completely

    MiniCPM4 layers: only quantize MLP (gate/up/down)
      - Attention stays BF16 (softmax attention anchor layers)
      - These 8 layers provide stable attention accuracy
    """
    if mixer_type == "minicpm4":
        # MiniCPM4: only MLP gets quantized, attention stays BF16
        if "self_attn" in linear_name:
            return False
        return True
    else:
        # Lightning-attn: quantize everything
        return True
...
def build_exclude_modules(config) -> List[str]:
    ...
    Only excludes MiniCPM4 attention projections (BF16).
    Lightning-attn projections are ALL quantized.
    """
    exclude = []
    for i, mt in enumerate(config.mixer_types):
        ...
        # Lightning-attn: nothing excluded — all quantized
    return exclude
...
def compute_fused_global_scales(
    ...
    Lightning-attn: QKV fusion + gate_up fusion
    MiniCPM4: gate_up fusion only (no QKV — attention is BF16)
    """
    fuse_groups = {}

    # QKV fusion for lightning-attn (all attention projections quantized)
    if mixer_type in ("lightning", "lightning_attn", "lightning-attn"):
        qkv = [
...
def quantize_model(args):
    ...
    print(f"\n[INFO] Quantization Plan (Dense Flashinfer NVFP4):")
    n_full_fp4 = 0
    n_mlp_fp4 = 0
    for i in range(num_layers):
        ...
        if mt == "minicpm4":
            ...
            n_full_fp4 += 1
    print(f"  Summary: {n_full_fp4} full-FP4 (lightning) + {n_mlp_fp4} MLP-only-FP4 (minicpm4)")
    ...
    for layer_idx in range(num_layers):
        ...
        print(f"\n{'=' * 70}")
        tag = "ALL=FP4" if mixer_type != "minicpm4" else "attn=BF16, MLP=FP4"
...
def main():
    ...
    print("=" * 70)
    print("  Dense Flashinfer NVFP4 Quantizer for MiniCPM-SALA")
    print("  MiniCPM4: attn=BF16, MLP=FP4 | Lightning: ALL=FP4")
