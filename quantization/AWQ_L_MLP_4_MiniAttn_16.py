...
# ====================================================================
# S10  Per-Layer Quantisation
# ====================================================================

def _quantize_layer(layer, layer_idx, config, device, calib_data, hs_store, all_masks, all_pos_ids, args, quantized_tensors, original_tensors, quant_filter=None):
    """
    quant_filter: Optional callable (linear_name: str) -> bool
                  If provided, only linears where quant_filter(name) is True
                  get NVFP4 quantized. Others are saved as BF16.
                  If None, all linears are quantized.
    """
    linears = find_all_linears(layer)
    linear_names = sorted(linears.keys())

    # Split into quantize vs keep-bf16 based on filter
    if quant_filter is not None:
        quant_names = [n for n in linear_names if quant_filter(n)]
        bf16_names = [n for n in linear_names if not quant_filter(n)]
        print(f"  Linears to NVFP4: {quant_names}")
        print(f"  Linears to BF16:  {bf16_names}")
    else:
        quant_names = linear_names
        bf16_names = []
        print(f"  Linears: {quant_names}")

    # Save BF16 linears immediately
    for ln_name in bf16_names:
        ln_mod = linears[ln_name]
        for pname, param in ln_mod.named_parameters():
            full_name = f"model.layers.{layer_idx}.{ln_name}.{pname}"
            original_tensors[full_name] = param.data.cpu().clone()

    observers = {n: ActAwareObserver(linears[n].weight.shape[1], device) for n in quant_names}
    hooks = [linears[n].register_forward_hook(lambda m, i, o, name=n: observers[name].add_batch(i[0].data)) for n in quant_names]
    ...
    merge_groups = {
        "qkv": [n for n in quant_names if any(n.endswith(s) for s in ["q_proj", "k_proj", "v_proj"])],
        "gate_up": [n for n in quant_names if any(n.endswith(s) for s in ["gate_proj", "up_proj"])],
    }
    merged_names = set(sum(merge_groups.values(), []))
    standalone = [n for n in quant_names if n not in merged_names]

    per_linear_amax = {n: linears[n].weight.data.abs().max().item() for n in quant_names}
    fused_global_sf = {}
    ...
    # Save non-quantized params (layernorms etc, excluding already-saved bf16 linears)
    saved_prefixes = set(ln_name + ".weight" for ln_name in quant_names)
    for name, param in layer.named_parameters():
        full_name = f"model.layers.{layer_idx}.{name}"
        if full_name not in quantized_tensors and full_name not in original_tensors:
            already_saved = any(name.startswith(p) for p in saved_prefixes)
...
def save_checkpoint(quantized_tensors, original_tensors, args, config, exclude_patterns):
    ...
    if os.path.exists(cfg_path):
        ...
        print(f"  Patched config.json with quantization_config and {len(exclude)} exclude_modules.")
...
def _is_mlp_linear(name: str) -> bool:
    """Filter: True for MLP linears, False for attention linears."""
    mlp_suffixes = ("gate_proj", "up_proj", "down_proj")
    return any(name.endswith(s) for s in mlp_suffixes)
...
def quantize_model(args):
    ...
    for i, mt in enumerate(config.mixer_types):
        if i in bf16_override:
            continue
        if mt in ("lightning", "lightning_attn", "lightning-attn"):
            lightning_layers.add(i)
        else:
            minicpm4_layers.add(i)

    # For exclude_modules in checkpoint: only attention weights in minicpm4 layers
    # are excluded (kept BF16). MLP weights are quantized everywhere.
    exclude_patterns = [f"model.layers.{i}.*" for i in bf16_override]
    for i in minicpm4_layers:
        for proj in ["q_proj", "k_proj", "v_proj", "o_proj", "o_gate"]:
            exclude_patterns.append(f"model.layers.{i}.self_attn.{proj}.*")
    ...
    for layer_idx in range(num_layers):
        ...
        is_lightning = layer_idx in lightning_layers
        is_minicpm4 = layer_idx in minicpm4_layers

        if is_lightning:
            tag = "NVFP4 (all linears)"
            print(f"\n{'='*70}\n[Layer {layer_idx}/{num_layers-1}] {mixer_type} -> {tag}\n{'='*70}")
            layer = model.model.layers[layer_idx].to(device)
            layer.eval()
            _quantize_layer(layer, layer_idx, config, device, calib_data,
                           hs_store, all_masks, all_pos_ids, args,
                           quantized_tensors, original_tensors,
                           quant_filter=None)  # ← all linears

        elif is_minicpm4:
            tag = "NVFP4 MLP + BF16 Attn"
            print(f"\n{'='*70}\n[Layer {layer_idx}/{num_layers-1}] {mixer_type} -> {tag}\n{'='*70}")
            layer = model.model.layers[layer_idx].to(device)
            layer.eval()
            _quantize_layer(layer, layer_idx, config, device, calib_data,
                           hs_store, all_masks, all_pos_ids, args,
                           quantized_tensors, original_tensors,
                           quant_filter=_is_mlp_linear)  # ← MLP only
            ...
            layer = model.model.layers[layer_idx].to(device)
            layer.eval()
            print("  Keeping BF16 (sparse layer)")
            for name, param in layer.named_parameters():
                original_tensors[f"model.layers.{layer_idx}.{name}"] = param.data.cpu().clone()

        # Update hidden states (same for all paths)
        with torch.no_grad():
            for i in range(len(calib_data)):
                inp = hs_store.load(i, device)
    ...
    save_checkpoint(quantized_tensors, original_tensors, args, config, exclude_patterns)
    hs_store.cleanup()
...
def main():
    ...
    print("=" * 70)
    print("  Activation-Aware NVFP4 Quantiser for MiniCPM-SALA")
