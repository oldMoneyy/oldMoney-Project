...
def _quantize_layer(layer, layer_idx, config, device, calib_data, hs_store, all_masks, all_pos_ids, args, quantized_tensors, original_tensors):
    ...
    merge_groups = {
        "qkv": [n for n in linear_names if any(n.endswith(s) for s in ["q_proj", "k_proj", "v_proj"])],
...
def quantize_model(args):
    ...
    quant_layers = [i for i, mt in enumerate(config.mixer_types) if mt in ("lightning", "lightning_attn", "lightning-attn")]
    ...
    for layer_idx in range(num_layers):
        ...
        tag = "Act-Aware NVFP4" if is_quant else "BF16"
        ...
        if is_quant:
            _quantize_layer(layer, layer_idx, config, device, calib_data, hs_store, all_masks, all_pos_ids, args, quantized_tensors, original_tensors)
        else:
