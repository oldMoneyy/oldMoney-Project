...
if [ ! -d /opt/oldmoney/quantization/venv ]; then
  python -m venv /opt/oldmoney/quantization/venv
fi
...
mkdir -p /opt/oldmoney/vendor
rm -rf /opt/oldmoney/vendor/infllmv2_cuda_impl
rm -rf /opt/oldmoney/vendor/sparse_kernel
cp -a /opt/SGLang-MiniCPM-SALA/packages/infllmv2_cuda_impl /opt/oldmoney/vendor/
cp -a /opt/SGLang-MiniCPM-SALA/packages/sparse_kernel /opt/oldmoney/vendor/

echo "===== CLEAN infllmv2_cuda_impl ====="
cd /opt/oldmoney/vendor/infllmv2_cuda_impl
...
cd /opt/oldmoney/vendor/sparse_kernel
...
' > /opt/oldmoney/quantization/prepare_gptq_env.log 2>&1 &