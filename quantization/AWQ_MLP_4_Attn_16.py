#!/usr/bin/env python3
"""
"Golden Strategy" Activation-Aware NVFP4 Quantization for MiniCPM-SALA
======================================================================
Strategy:
  - All MLP layers -> NVFP4 (Saves ~70% memory)
  - All Attention layers -> BF16 (Preserves ~100% Accuracy)
  - Layers 26-28 -> Pure BF16 (Late-layer protection)
"""
...
def _is_mlp_linear(name: str) -> bool:
    """GOLDEN FILTER: True ONLY for MLP linears. Attention linears return False."""
...
def _quantize_layer(layer, layer_idx, config, device, calib_data, hs_store, all_masks, all_pos_ids, args, quantized_tensors, original_tensors):
    ...
    # Only quantize MLP, strictly protect Attention
    quant_names = [n for n in linear_names if _is_mlp_linear(n)]
    bf16_names = [n for n in linear_names if not _is_mlp_linear(n)]
    
    print(f"  Linears to NVFP4 (MLP):  {quant_names}")
    print(f"  Linears to BF16 (Attn):  {bf16_names}")
    ...
    print(f"  Running Act-Aware MSE-RTN...")
...
def save_checkpoint(quantized_tensors, original_tensors, args, config, exclude_patterns):
    ...
    if os.path.exists(cfg_path):
        ...
        print(f"  Patched config.json with quantization_config and {len(exclude_patterns)} exclude_modules.")
...
def quantize_model(args):
    ...
    # 1. Protect late layers entirely in BF16
    bf16_override = set()
    if args.bf16_late_layers:
        bf16_override = {26, 27, 28}
        print(f"[INFO] Keeping layers {sorted(bf16_override)} entirely in BF16")

    # 2. Generate EXCLUDE PATTERNS for the Golden Strategy
    exclude_patterns = []
    for i in bf16_override:
        exclude_patterns.append(f"model.layers.{i}.*")
        
    for i in range(num_layers):
        if i not in bf16_override:
            # Tell SGLang to KEEP Attention as BF16 for all other layers
            exclude_patterns.append(f"model.layers.{i}.self_attn.*")
    ...
    for layer_idx in range(num_layers):
        ...
        if layer_idx in bf16_override:
            tag = "Pure BF16 (Protected)"
            ...
            tag = "Golden Strategy (NVFP4 MLP + BF16 Attn)"
            print(f"\n{'='*70}\n[Layer {layer_idx}/{num_layers-1}] {mixer_type} -> {tag}\n{'='*70}")
            layer = model.model.layers[layer_idx].to(device)
            layer.eval()
    ...
    hs_store.cleanup()
    print("\n[DONE] Golden Strategy NVFP4 quantisation complete!")
...
def main():
    ...
    parser.add_argument("--bf16-late-layers", action="store_true",
                        help="Keep layers 26-28 in BF16 completely")
    args = parser.parse_args()

    print("=" * 70)
    print("  GOLDEN STRATEGY NVFP4 Quantiser for MiniCPM-SALA")
    print("  (ALL MLP = NVFP4 | ALL Attention = BF16)")
    print("=" * 70)
