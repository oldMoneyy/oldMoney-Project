#!/usr/bin/env python3
"""
GPTQ W4A16 quantization for MiniCPM-SALA with SmoothQuant channel equalization.

Based on GPTQ_int4_flashinfer_dense_gpu.py with these improvements:
1. Per-sample normalized Hessian accumulation (equal weighting regardless of seq len)
2. SmoothQuant-style channel equalization (4 groups: attn, o_norm, MLP, up→down)
3. Deterministic calibration (seed=42, no shuffle)
4. GPU-native Hessian (float64 on GPU for speed)
...
class GPTQ:
    ...
    def __init__(self, name: str, weight: torch.Tensor):
        ...
        # GPU-native Hessian in float64 for speed + stability
        self.H = torch.zeros(
    ...
    def add_batch(self, inp: torch.Tensor):
        ...
        inp = inp.float()
        # Per-sample normalization: each sample contributes equally
        h_update = (inp.T @ inp) / max(n_tokens, 1)
        self.H.add_(h_update.to(torch.float64))
        self.nsamples += 1  # count samples, not tokens
...
def compute_smooth_factor(
    ...
    """Compute SmoothQuant channel equalization factor.

    s[j] = act_scale[j]^alpha / weight_scale[j]^(1-alpha), normalized so mean(s)=1.
    """
...
def get_submodule_safe(module: nn.Module, path: str):
    """Get a submodule by dot-separated path, returning None if not found."""
    parts = path.split(".")
    current = module
    for part in parts:
...
def apply_smoothing(
    ...
    """Apply SmoothQuant channel equalization to a layer.

    4 smoothing groups (matching NVFP4 AWQ script):
    ...
    if (up_name in linears and down_name in linears
        ...
        # up_proj output dim / down_proj input dim
...
def main():
    ...
    print("=" * 70)
    print("GPTQ Quantizer for MiniCPM-SALA (Dense + SmoothQuant)")
    print("=" * 70)
