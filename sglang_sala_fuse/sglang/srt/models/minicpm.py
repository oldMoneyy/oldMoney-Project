...
class MiniCPMDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # --- Input layernorm (fused with scaled-add from previous layer if possible) ---
        if residual is None:
            # First layer: standalone RMSNorm, hidden_states becomes residual
            residual = hidden_states
            hidden_states = self.input_layernorm(hidden_states)
        else:
            # Subsequent layers: fuse residual + hidden_states * scale + RMSNorm
        ...
        # --- Fused: residual + attn_output * scale + post_attention_layernorm ---
        hidden_states, residual = fused_scaled_add_rmsnorm(
            hidden_states, residual,
...
class MiniCPMModel(nn.Module):
    ...
    def forward(
        ...
        # Final scaled add from last layer's MLP output + final RMSNorm
        scale = self.config.scale_depth / math.sqrt(self.config.num_hidden_layers)
